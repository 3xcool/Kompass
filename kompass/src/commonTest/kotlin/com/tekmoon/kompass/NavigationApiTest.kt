...
class NavigationApiTest {
    ...
    @Test fun replacing_the_stack_publishes_one_state_not_one_per_level() {
        ...
        try {
            seen.clear()
            nav.replaceStack(listOf(A.toKompassBackStackEntry(), B.toKompassBackStackEntry(), A.toKompassBackStackEntry()))
    ...
    @Test fun a_replaced_stack_keeps_every_level_a_separate_occurrence() {
        ...
        try {
            ...
            val reused = B.toKompassBackStackEntry()
    ...
    @Test fun replacing_the_stack_with_one_entry_clears_the_rest() {
        ...
        try {
            ...
            nav.replaceStack(B.toKompassBackStackEntry())
    ...
    @Test fun a_saved_stack_restores_through_one_command() {
        ...
        try {
            source.navigate(B.toKompassBackStackEntry(args = "opaque"))
    ...
    @Test fun consume_is_typed_once_and_targets_the_occurrence() {
        val root = A.toKompassBackStackEntry(results = mapOf("answer" to Answer("root"), "keep" to Answer("keep")))
        val duplicate = A.toKompassBackStackEntry(results = mapOf("answer" to Answer("duplicate")))
    ...
    @Test fun delivered_result_can_be_consumed_after_restore() {
        ...
        try {
            nav.navigate(B.toKompassBackStackEntry(pendingResultKey = "answer"))
    ...
    @Test fun duplicate_saved_identity_and_unregistered_results_are_reported() {
        val entry = A.toKompassBackStackEntry()
        ...
        val source = createKompassNavController(defaultNavigationState(A.toKompassBackStackEntry(results = mapOf("a" to Answer("x")))), serializers)
        try {
    ...
    @Test fun direction_comes_from_commands_and_result_consumption_does_not_change_it() {
        ...
        try {
            ...
            nav.navigate(B.toKompassBackStackEntry(), reuseIfExists = true)
            assertEquals(listOf("a", "b"), nav.backStack.map { it.destinationId })
            assertEquals(NavDirection.Push, nav.direction)
            nav.navigate(A.toKompassBackStackEntry(), clearBackStack = true)
            assertEquals(NavDirection.Push, nav.direction)
        } finally { nav.close() }
    }
    ...
    @Test fun external_flow_observes_commands_without_composition_and_close_rejects_mutation() {
        ...
        try {
            ...
            nav.close()
            nav.close()
            assertFailsWith<IllegalStateException> { nav.navigate(B.toKompassBackStackEntry()) }
    ...
    @Test fun external_controller_keeps_deep_links_typed_arguments_and_command_order() {
        val handler = object : DeepLinkHandler {
            ...
            override fun resolve(uri: String) = listOf(NavigationCommand.ReplaceRoot(A.toKompassBackStackEntry()), NavigationCommand.Navigate(B.toKompassBackStackEntry(args = "opaque")))
        }
